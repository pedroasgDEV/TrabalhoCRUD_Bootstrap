<?php
    include_once("connectDB.php");
    $id = $_POST['id'];
    $nomeUser=$_POST['nomeUser'];
    $niverUser=$_POST['niverUser'];
    $cpf=$_POST['cpf'];
    $cep=$_POST['cep'];
    $estadoUser=$_POST['estadoUser'];
    $cityUser=$_POST['cityUser'];
    $baiUser=$_POST['baiUser'];
    $ruaUser=$_POST['ruaUser'];
    $numbUser=$_POST['numbUser'];
    $salUser=$_POST['salUser'];
    $sexoUser=$_POST['sexoUser'];
    $telUser=$_POST['telUser'];
    $emailUser=$_POST['emailUser'];
    $confirmPassUser=$_POST['confirmPassUser'];
    $habitatUser=$_POST['habitatUser'];
    $motivUser=$_POST['motivUser'];
    $sql="UPDATE pessoa SET cpf='$cpf',nome='$nomeUser',cep='$cep', sexo='$sexoUser', estado='$estadoUser', cidade='$cityUser',bairro='$baiUser', rua='$ruaUser', numb='$numbUser', habitat='$habitatUser', motivos='$motivUser', aniver='$niverUser', salmin='$salUser', email='$emailUser', telefone='$telUser', senha='$confirmPassUser' WHERE id = '$id'";
    $criarest= mysqli_query($conectar,$sql);
	header("Location:../homeUser/userPerfil.php?id=".$id);
?> 